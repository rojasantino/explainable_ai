# Angular Frontend — Complete Code

## Setup Commands
```bash
npm install -g @angular/cli
ng new customer-ai-frontend --routing --style=css
...
export class ApiService {
  ...
  predict(userId: number): Observable<any> {
    return this.http.get(`${this.base}/predict/${userId}`);
  }
  recommend(userId: number): Observable<any> {
    return this.http.get(`${this.base}/recommend/${userId}`);
  }
  explain(userId: number): Observable<any> {
    return this.http.get(`${this.base}/explain/${userId}`);
  }
  getSegment(userId: number): Observable<any> {
    return this.http.get(`${this.base}/segment/${userId}`);
  }
...
## 4. src/app/navbar/navbar.component.ts
...
@Component({
  ...
  styles: [`
    ...
    .links a { color:rgba(255,255,255,0.85); text-decoration:none;
               margin-left:20px; font-size:14px; }
    .links a.active { color:white; font-weight:600;
...
## 5. src/app/dashboard/dashboard.component.ts
...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getDashboardStats().subscribe({
      ...
      error: (err) => { this.error = 'Cannot connect to API'; this.loading = false; }
    });
  }
  ...
  getSegmentColor(seg: string): string {
    const map: any = {
      'High Value': '#185FA5', 'Loyal': '#3B6D11',
      'At Risk': '#A32D2D',   'New Customer': '#854F0B'
    };
    return map[seg] || '#888';
  }
}
      ...
      <div class="segment-grid">
        <div class="segment-pill"
             *ngFor="let seg of stats.segments | keyvalue"
             [style.background]="getSegmentColor(seg.key)">
...
.error-msg    { background: #FCEBEB; color: #A32D2D; padding: 12px 16px;
...
.section-card { background: white; border: 0.5px solid #ddd;
...
.segment-pill { border-radius: 20px; padding: 8px 16px; color: white;
...
.prod-id      { width: 60px; font-size: 13px; font-weight: 500; }
.bar-wrap     { flex: 1; background: #eee; border-radius: 4px; height: 8px; }
.bar-fill     { background: #185FA5; border-radius: 4px; height: 100%; transition: width .4s; }
...
## 6. src/app/personalization/personalization.component.ts
...
export class PersonalizationComponent {
  ...
  search(): void {
    ...
    this.api.getCustomerProfile(this.userId).subscribe({
      ...
      error: () => {
        this.error   = `User ${this.userId} not found.`;
        this.loading = false;
      }
    });
  }
  ...
  getSegmentBadgeStyle(seg: string): object {
    const colors: any = {
      'High Value': '#185FA5', 'Loyal': '#3B6D11',
      'At Risk': '#A32D2D',   'New Customer': '#854F0B'
    };
    return { background: colors[seg] || '#888', color: 'white',
...
## 6b. personalization.component.html
...
<div class="page">
  ...
  <div class="search-box">
    <input type="number" [(ngModel)]="userId"
           placeholder="Enter Customer ID (101–600)"
           (keyup.enter)="search()" />
    ...
    <div class="section-card">
      <div class="profile-header">
        ...
        <div>
          ...
          <div class="profile-meta">
            {{ profile.age }} yrs | {{ profile.gender }} | {{ profile.location }}
          </div>
          <span [ngStyle]="getSegmentBadgeStyle(profile.segment)">
            {{ profile.segment }}
          </span>
        </div>
      </div>
      <div class="stats-row">
        <div class="stat"><div class="stat-label">Orders</div>
             <div class="stat-value">{{ profile.total_orders }}</div></div>
        <div class="stat"><div class="stat-label">Total Spent</div>
             <div class="stat-value">₹{{ profile.total_spend | number:'1.0-0' }}</div></div>
        <div class="stat"><div class="stat-label">Buy Probability</div>
             <div class="stat-value"
                  [style.color]="getProbabilityColor(profile.buy_probability)">
               {{ (profile.buy_probability * 100).toFixed(0) }}%
             </div></div>
      </div>
    </div>
    ...
    <div class="section-card">
      ...
      <div class="prob-bar-wrap">
        <div class="prob-bar-fill"
             [style.width.%]="profile.buy_probability * 100"
             [style.background]="getProbabilityColor(profile.buy_probability)">
        </div>
      </div>
      <p class="prob-label">
        {{ (profile.buy_probability * 100).toFixed(1) }}% chance of purchasing
      </p>
    </div>
...
.avatar       { width: 52px; height: 52px; border-radius: 50%; background: #185FA5;
...
.rec-card     { background: #EAF3DE; border-radius: 8px; padding: 10px 8px;
                text-align: center; }
...
.reason       { display: flex; align-items: flex-start; gap: 10px;
                padding: 9px 0; border-bottom: 0.5px solid rgba(24,95,165,0.2);
...
## Run Flask API (separate terminal)
```bash
cd customer-ai
python api/app.py
